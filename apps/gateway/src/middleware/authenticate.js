...
import { redis } from "@micro/db/conn";

const SESS_PREFIX = "sess:"; // sess:<sub> -> jti (current session id)
const JWT_SECRET = process.env.JWT_SECRET;
if (!JWT_SECRET) throw new Error("JWT_SECRET is required");

const JWT_ISSUER = process.env.JWT_ISSUER || "micro-auth";
const JWT_AUDIENCE = process.env.JWT_AUDIENCE || "micro-gateway";
...
export async function authenticate(req, res, next) {
  ...
  try {
    ...
    return res.status(401).json({
      message: isExpired ? "Token expired" : "Invalid token deo biet luon ne",
    });
  }
}
